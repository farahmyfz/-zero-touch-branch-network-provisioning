<script>
  ...
  import { goto } from '$app/navigation'; // Tambahkan untuk navigasi

  // --- Proteksi Halaman & Logout ---
  onMount(() => {
    // Cek status login di browser
    const isAuthenticated = localStorage.getItem('isAuthenticated');
    if (!isAuthenticated) {
      goto('/login'); // Tendang ke login jika coba bypass URL
    }
  });
...
<div class="min-h-screen bg-[#F8FAFC] text-slate-700 font-sans selection:bg-emerald-100">
  ...
  <header class="bg-white px-8 py-3 flex justify-between items-center border-b border-slate-200 shadow-sm sticky top-0 z-50">
    ...
    <div class="flex items-center gap-6">
      ...
      <button on:click={handleLogout} class="text-[10px] font-bold text-slate-400 hover:text-red-500 uppercase tracking-widest transition-colors">
        Logout
      </button>
    </div>
  </header>
    ...
    <div class="col-span-4 space-y-6">
      ...
      <div class="bg-white border border-slate-200 rounded-[2rem] shadow-xl shadow-slate-200/50 flex flex-col h-[765px] overflow-hidden">
        ...
        <div class="flex-1 overflow-y-auto p-6 space-y-4 bg-[#FCFDFF] custom-scrollbar">
          {#each alerts as a}
            <div class="group border-l-4 {a.type === 'SECURITY' ? 'border-red-500 bg-red-50/50' : 'border-emerald-500 bg-emerald-50/50'} p-4 rounded-r-2xl transition-all hover:translate-x-1 shadow-sm">
